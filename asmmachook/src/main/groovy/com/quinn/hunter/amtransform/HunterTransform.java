package com.quinn.hunter.amtransform;
...
import com.quinn.hunter.amtransform.asm.ClassLoaderHelper;
...
public class HunterTransform extends Transform {
            ...
            for (JarInput jarInput : input.getJarInputs()) {
                ...
                if (isIncremental && !emptyRun) {
                    switch (status) {
                        ...
                        case ADDED:
                        case CHANGED:
                            transformJar(jarInput.getFile(), dest, status, jarInput);
                            break;
                        case REMOVED:
                            if (dest.exists()) {
                    ...
                    for (Map.Entry<File, Status> changedFile : fileStatusMap.entrySet()) {
                        ...
                        switch (status) {
                            ...
                            case CHANGED:
                                ...
                                transformSingleFile(inputFile, destFile, srcDirPath, directoryInput);
                                break;
                        }
                    }
                } else {
                    transformDir(directoryInput.getFile(), dest, directoryInput);
                }
            }
        }
    ...
    private void transformSingleFile(final File inputFile, final File outputFile, final String srcBaseDir, final QualifiedContent input) {
        waitableExecutor.execute(() -> {
            bytecodeWeaver.weaveSingleClassToFile(inputFile, outputFile, srcBaseDir, input);
            return null;
        });
    }

    private void transformDir(final File inputDir, final File outputDir, QualifiedContent input) throws IOException {
        if (emptyRun) {
        ...
        if (inputDir.isDirectory()) {
            for (final File file : com.android.utils.FileUtils.getAllFiles(inputDir)) {
                waitableExecutor.execute(() -> {
                    ...
                    bytecodeWeaver.weaveSingleClassToFile(file, outputFile, inputDirPath, input);
                    return null;
                });
            }
        }
    ...
    private void transformJar(final File srcJar, final File destJar, Status status, QualifiedContent input) {
        waitableExecutor.execute(() -> {
            if (emptyRun) {
                FileUtils.copyFile(srcJar, destJar);
                return null;
            }
            bytecodeWeaver.weaveJar(srcJar, destJar, input);
            return null;
        });
    }
